...
class TenantUsers:
    def __init__(
            self,
            result: canopy.swagger.GetTenantUsersQueryResult):
    ...
    @property
    def list(self) -> Sequence[canopy.swagger.GetTenantUsersQueryResultUserItem]:
        return self._list

    def get_by_user_id(self, user_id: str) -> Optional[canopy.swagger.GetTenantUsersQueryResultUserItem]:
        if user_id in self._by_user_id:
            return self._by_user_id[user_id]
        else:
            raise canopy.NotFoundError('Unknown user ID ' + user_id)

    def get_by_username(self, username: str) -> Optional[canopy.swagger.GetTenantUsersQueryResultUserItem]:
